...
with open(variantFilename) as infile:
    ...
    for row in reader:
        ...
        if pValue < 0.01:
            ...
            # Avoid duplicate reporting, which happens if multiple overlapping ORF's are 
            # present in the GFF file provided to iVar.
            ...
            # Construct a mutation label based on the reporting format of the mutations in constellations
            if (len(altAA)==1) and (ORFname!="NUC") and (refAA!=altAA):
                # Mutations causing residue substitutions in CDS
                mutationLabel = ''.join( (ORFname, ':', refAA, str(mutationPosORF), altAA) )
            else:
                if (len(altBase)==1) or (altBase[0]=='+'):
                    # Insertions (+A +AT etc.) OR substitutions in UTR OR silent residue substitutions in CDS             
                    mutationLabel = ''.join( ('NUC:', refBase, str(mutationPos), altBase) )
                else:
                    # Deletions in CDS or UTR
                    mutationLabel = ''.join( ('DEL:', str(mutationPos), ':', str(len(altBase)-1) ) )
            ...
            if mutationLabel in uniqueMutationLabels:
                ...
                for varname in mut2var[mutationLabel]:
                    compatibleVars.append("<a href=\"https://outbreak.info/situation-reports?pango=" + varname + "\">" + varname + "</a>")
                compatibleVars = ', '.join(compatibleVars)
            else:
                compatibleVars = "None found"
            
            # If present at a significant level, then include among the mutations to be shown on the report file.
            if altFreq >= 0.05:
                outfile.write("<tr>\n")
                outfile.write("    <td>%d</td> <td>%s</td> <td>%s</td> <td>%.3f</td> <td nowrap>%.2E</td> <td>%s</td> <td>%s</td>\n" 
...
def buildVarSupportTable (varGenre):
    ...
    for varname in importantVars[varGenre]:
        outfile.write("<td>%s</td>" % ("<a href=\"https://outbreak.info/situation-reports?pango=" + varname + "\">" + varname + "</a>") )
        
    outfile.write("</tr>\n<tr>\n    <td>Characteristic mutations <br> detected</td>")
    for varname in importantVars[varGenre]:
        mutSupport = isVarSupported(varname)   
        outfile.write("<td>")
        outfile.write('(%d of %d)<br>' % (len(mutSupport['supporting']), len(mutSupport['supporting'])+len(mutSupport['unsupporting']) ))
