...
with open(pileupFilename) as infile:
    ...
    for row in reader:
        pos = int(row[1])-1
        if pos >= GENOME_SIZE:
            print('WARNING: Pileup file contains more rows than the genome length. Omitted extra fields.')
            break
...
plt.xticks( np.arange(0,GENOME_SIZE, 5000), [ "%d" % (x/1000) for x in np.arange(0,GENOME_SIZE, 5000)] )


# Add shading for uncovered regions
ymax = plt.gca().get_ylim()[1]
for i in range(0,gapCounter):
    plt.fill( [gapStart[i],gapEnd[i],gapEnd[i],gapStart[i]], [0,0,ymax,ymax], 'r', alpha=1, edgecolor='none')
    plt.text( (gapStart[i]+gapEnd[i])/2-GENOME_SIZE/60, 1.01*ymax, '*', color='r', weight='bold', size=20)
...
if (30-xmin) < 0.15*(xmax-xmin):
    plt.text(xmin, 1.05*ymax, "%.1f%%" % pctBelowThreshold,  color='r', size=16)
else:
    plt.text(10, 1.05*ymax, "%.1f%%" % pctBelowThreshold,  color='r', size=16)
plt.text((30+xmax)/2-5, 1.05*ymax, "%.1f%%" % (100-pctBelowThreshold),  color='g', size=16)
...
if np.mean(readDepth) > 500:
    ...
    plt.plot( posIdx[readDepth<100], readDepth[readDepth<100]/1000, '.', color='k')
...
if np.mean(readDepth) > 1000:
    plt.hist(readDepth/1000, bins=50, color=FDAblue, edgecolor=None, density=True )
    (ymin,ymax) = plt.gca().get_ylim()
    ...
    if (0.1-xmin) < 0.15*(xmax-xmin):
        ...
        plt.text(0.8*(xmin+0.1)/2, 1.05*ymax, "%.1f%%" % pctBelowThreshold,  color='r', size=16)
    plt.text(max(0.09,0.8*(xmax+0.1)/2), 1.05*ymax, "%.1f%%" % (100-pctBelowThreshold),  color='g', size=16)
else:
    ...
    if (100-xmin) < 0.15*(xmax-xmin):
        ...
        plt.text(0.8*(xmin+100)/2, 1.05*ymax, "%.1f%%" % pctBelowThreshold,  color='r', size=16)
    plt.text(max(90,0.8*(xmax+100)/2), 1.05*ymax, "%.1f%%" % (100-pctBelowThreshold),  color='g', size=16)
...
with open(outfilename, 'w') as outfile:
    ...
    for i in range(0,GENOME_SIZE):
        writer.writerow([ "%d" % posIdx[i], "%d" % readDepth[i], '%.2f' % quality[i] ])
